...
* 주의할 점
  ...
  * 트랜잭션은 꼭 필요한 최소의 코드로 이루어져야 함
    ...
    * 왜냐면 DB 커넥션은 한정적이기 때문에, 커넥션 소유 시간이 길어지면 다른 작업이 커넥션을 기다려야하는 상태 발생
...
* Statement
  ...
  * 과정
    1. 구문 분석 (Parse)
       - 입력 데이터 넣고 쿼리 문법 검사 & 쿼리 컴파일 & 쿼리 실행 계획
       - 선정된 실행 계획을 캐시에 저장
    3. 실행 (execute)
       - 디스크에서 블록을 찾아, 버퍼에 복사
    4. 인출 (fetch)
...
* PreparedStatement
  ...
  * 미리 컴파일된 SQL문을 캐시에 보관하고, 이후에 데이터를 대입함으로써, 반복 작업 시 속도 향상

  * 장점

    * 같은 명령일 경우, 구문 분석 다시 하지 않고, 데이터 바인딩, 실행, 인출만 거침
...
* 격리 수준 (Isolation Level)
  ...
  * 필요성
    * Locking을 통해 트랜잭션 간 서로 간섭 못하게 해야하지만
    * 무조건 Lock써서 트랜잭션을 순차처리하는 건 성능 저하 유발
...
* 종류
  ...
  * Repeatable Read (Lv.2)
    ...
    * 자신의 트랜잭션 번호보다 낮은 번호의 트랜잭션에서 커밋된 내용만 봄
